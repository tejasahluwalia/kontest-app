...
import { createEffect, createMemo, createSignal, ErrorBoundary, For } from "solid-js";
import { useFormBuilder } from "../form-builder-context";
import type { ConditionalRule } from "../primitives/conditions";
import type { Edges } from "../primitives/edges";
import { createDefaultStepNodeGraph, type StepGraphNode } from "../primitives/form";
...
function StepNode(props: {
  ...
  createEffect(() => {
    updateStepInGraph(props.node.step.id, { step:{...props.node.step, label: label()} });
  });
    ...
    >
      <CardContent class="p-4">
        ...
        <div class="space-y-2 mt-4">
          <div class="flex items-center">
            <h4 class="text-sm font-medium text-muted-foreground">Connections</h4>
          ...
          <div class="space-y-2">
            <For each={props.node.edges}>
              {(edge) => {
                const targetStep = props.allNodes.find(n => n.step.id === edge.action.id);
                return (
                  <div class="flex items-center gap-2">
                    <ArrowRight class="h-4 w-4 text-muted-foreground" />
                    <span class="flex-1">{targetStep ? targetStep.step.id : 'Unknown Step'}</span>
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={() => props.onRemoveEdge(edge.action.id, props.node.step.id)}
                    >
                      <Trash class="h-3 w-3" />
                    </Button>
                  </div>
