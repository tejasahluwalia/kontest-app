...
export function FieldPropertiesPanel() {
  const { formSchema, activeFieldId, activeSection, updateField, removeField } = useFormBuilder();
  const [activeTab, setActiveTab] = createSignal("general");

  const activeField = () => {
    if (!activeFieldId() || !activeSection()) return null;
    
    const section = formSchema.sections.find(s => s.id === activeSection());
    if (!section) return null;
    
    return section.fields.find(f => f.id === activeFieldId());
  };

  const handleUpdateField = (data: Partial<FormField>) => {
    if (activeField() && activeSection()) {
      updateField(activeSection()!, activeFieldId()!, data);
    }
  };
  ...
  const handleDeleteField = () => {
    if (activeField() && activeSection()) {
      removeField(activeSection()!, activeFieldId()!);
    }
  };

  return (
    <Show when={activeField()} fallback={
      <Card>
        <CardHeader>
        ...
        <CardContent>
          <Tabs value={activeTab()} onChange={setActiveTab}>
            <TabsList class="w-full">
              ...
              <Show when={["select", "multiselect", "checkbox", "radio"].includes(activeField()?.type || "")}>
            ...
            <TabsContent value="validation" class="space-y-4 pt-4">
              ...
              <Show when={activeField()?.type === "number"}>
                <div>
              ...
              <Show when={["text", "textarea"].includes(activeField()?.type || "")}>
                <div>
              ...
              <div>
                ...
                <Input
                  ...
                  value={activeField()?.validation?.customMessage || ""}
                  onInput={(e) => {
                    handleUpdateField({ 
                      validation: { 
                        ...activeField()?.validation,
