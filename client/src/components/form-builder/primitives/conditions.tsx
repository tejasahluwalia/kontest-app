...
function evaluateSingleCondition(
	...
	const leftValue = getConditionValue(
		condition.leftType,
		condition.leftValue,
		formData,
	);
	const rightValue = getConditionValue(
		condition.rightType,
...
const SingleCondition: Component<{
	...
	const [leftConstant, setLeftConstant] = createSignal(
		props.condition().leftValue,
	);
	const [rightConstant, setRightConstant] = createSignal(
		props.condition().rightValue,
	);
	return (
		<div class="flex items-center gap-2 mb-2">
			<div class="flex-1 flex items-center gap-2">
				<div class="grid">
					<Select
						options={VALUE_TYPES}
						value={VALUE_TYPES.find(
							(vt) => vt.value === props.condition().leftType,
						)}
						optionTextValue="label"
						optionValue="value"
						onChange={(value) => {
							if (value)
								props.onUpdate({ ...props.condition(), leftType: value.value });
						}}
						placeholder="Select type"
						itemComponent={(props) => (
							<SelectItem class="uppercase" item={props.item}>
					...
					>
						<SelectTrigger class="uppercase" aria-label="Select type">
							<SelectValue<{ label: string; value: ValueType }>>
					...
					>
						<Select
							...
							value={props.formValues.find(
								(fv) => fv.value === props.condition().leftValue,
							)}
							optionTextValue="label"
							optionValue="value"
							onChange={(value) => {
				...
				<Select
					...
					value={COMPARISON_OPERATORS.find(
						(co) => co.value === props.condition().operator,
					)}
					optionTextValue="label"
					optionValue="value"
					onChange={(value) => {
						if (value)
							props.onUpdate({ ...props.condition(), operator: value.value });
					}}
				...
				>
					<SelectTrigger class="uppercase" aria-label="Select operator">
						<SelectValue<{ label: string; value: ComparisonOperator }>>
							{(state) => state.selectedOption()?.label}
						</SelectValue>
					</SelectTrigger>
					<SelectContent />
				...
				<Select
					options={VALUE_TYPES}
					value={VALUE_TYPES.find(
						(vt) => vt.value === props.condition().rightType,
					)}
					optionTextValue="label"
					optionValue="value"
					onChange={(value) => {
						if (value)
							props.onUpdate({ ...props.condition(), rightType: value.value });
					}}
					placeholder="Select type"
					class="w-32"
					itemComponent={(props) => (
				...
				>
					<Select
						options={props.formValues}
						value={props.formValues.find(
							(fv) => fv.value === props.condition().rightValue,
						)}
						optionTextValue="label"
						optionValue="value"
						onChange={(value) => {
...
export const ConditionalLogic: Component<ConditionalLogicProps> = ({
	rules,
	onChange,
	actions,
	formValues,
					...
					return (
						<div class="border rounded-md p-4">
							...
							<ConditionGroupComponent
								...
								onUpdate={(updatedGroup) =>
									updateRule(index(), { ...rule, condition: updatedGroup })
								}
							...
							<div class="mt-4 flex items-center gap-2">
								...
								>
									<SelectTrigger
										aria-label="Select destination step"
										class="w-60"
									>
										<SelectValue<Action>>
