import { DialogContent, DialogHeader, DialogTitle } from "@client/components/ui/dialog";
...
const FieldPropertiesModal: ParentComponent<{field: Accessor<InputField>, blockId: string}> = (props) => {
    const {
        updateChildInBlock,
      } = useFormBuilder();    const [activeTab, setActiveTab] = createSignal("general");
    const { field, blockId } = props;

    const handleUpdateField = (data: Partial<InputField>) => {
        updateChildInBlock(field().id, blockId, data);
      };
        ...
        <DialogContent>
            ...
            <Tabs value={activeTab()} onChange={setActiveTab}>
                <TabsList class="w-full mb-4">
                  ...
                  <Show when={["select", "checkbox", "radio"].includes((field().fieldType || '').toLowerCase())}>
                  ...
                  <Show when={field().defaultValue !== undefined}>
                    <div>
                      ...
                      <Input
                        ...
                        value={typeof field().defaultValue === 'object' ? JSON.stringify(field().defaultValue) : String(field().defaultValue || "")}
                ...
                <TabsContent value="validation" class="space-y-4">
                  ...
                  <Show when={["text", "rich-text"].includes(field().fieldType.toLowerCase())}>
                    <div>
                ...
                <TabsContent value="options" class="space-y-4">
                  <Show when={(() => {
                    ...
                    {(fieldWithOptions) => <div class="space-y-2">
