import { defineRelations } from "drizzle-orm";
import {
	boolean,
	jsonb,
	pgEnum,
	pgTable,
	primaryKey,
	text,
	timestamp,
	unique,
} from "drizzle-orm/pg-core";
import { nanoid } from "nanoid";

export const user = pgTable("user", {
	id: text("id").primaryKey(),
	name: text("name").notNull(),
	email: text("email").notNull().unique(),
	emailVerified: boolean("email_verified").notNull(),
	image: text("image"),
	createdAt: timestamp("created_at").notNull(),
	updatedAt: timestamp("updated_at").notNull(),
});

export const session = pgTable("session", {
	id: text("id").primaryKey(),
	expiresAt: timestamp("expires_at").notNull(),
	token: text("token").notNull().unique(),
	createdAt: timestamp("created_at").notNull(),
	updatedAt: timestamp("updated_at").notNull(),
	ipAddress: text("ip_address"),
	userAgent: text("user_agent"),
	userId: text("user_id")
		.notNull()
		.references(() => user.id, { onDelete: "cascade" }),
});

export const account = pgTable("account", {
	id: text("id").primaryKey(),
	accountId: text("account_id").notNull(),
	providerId: text("provider_id").notNull(),
	userId: text("user_id")
		.notNull()
		.references(() => user.id, { onDelete: "cascade" }),
	accessToken: text("access_token"),
	refreshToken: text("refresh_token"),
	idToken: text("id_token"),
	accessTokenExpiresAt: timestamp("access_token_expires_at"),
	refreshTokenExpiresAt: timestamp("refresh_token_expires_at"),
	scope: text("scope"),
	password: text("password"),
	createdAt: timestamp("created_at").notNull(),
	updatedAt: timestamp("updated_at").notNull(),
});

export const verification = pgTable("verification", {
	id: text("id").primaryKey(),
	identifier: text("identifier").notNull(),
	value: text("value").notNull(),
	expiresAt: timestamp("expires_at").notNull(),
	createdAt: timestamp("created_at").notNull(),
	updatedAt: timestamp("updated_at").notNull(),
});

export const org = pgTable("org", {
	id: text("id")
		.$defaultFn(() => nanoid())
		.primaryKey(),
	name: text("name").notNull(),
	slug: text("slug").notNull().unique(),
	metadata: jsonb("metadata"),
	createdAt: timestamp("created_at").defaultNow().notNull(),
	updatedAt: timestamp("updated_at").defaultNow().notNull(),
});

export const memberRoleEnum = pgEnum("org_member_role", ["admin", "member"]);

export const member = pgTable(
	"member",
	{
		id: text("id")
			.$defaultFn(() => nanoid())
			.notNull()
			.primaryKey(),
		userId: text("user_id")
			.notNull()
			.references(() => user.id, { onDelete: "cascade" }),
		orgId: text("org_id")
			.notNull()
			.references(() => org.id, { onDelete: "cascade" }),
		role: memberRoleEnum("org_member_role").notNull().default("member"),
		createdAt: timestamp("created_at").defaultNow().notNull(),
		updatedAt: timestamp("updated_at").defaultNow().notNull(),
	},
	(t) => [unique().on(t.orgId, t.userId)],
);

export const round = pgTable("round", {
	id: text("id")
		.$defaultFn(() => nanoid())
		.primaryKey(),
	name: text("name").notNull(),
	slug: text("slug").notNull().unique(),
	callId: text("call_id")
		.notNull()
		.references(() => call.id, { onDelete: "cascade" }),
	formSchema: jsonb("form_schema"),
	judgingSchema: jsonb("judging_schema"),
	metadata: jsonb("metadata"),
	startDate: timestamp("start_date"),
	endDate: timestamp("end_date"),
	createdAt: timestamp("created_at").defaultNow().notNull(),
	updatedAt: timestamp("updated_at").defaultNow().notNull(),
});

export const callVisibilityEnum = pgEnum("call_visibility", [
	"public",
	"private",
	"restricted",
]);

export const call = pgTable("call", {
	id: text("id")
		.$defaultFn(() => nanoid())
		.primaryKey(),
	name: text("name").notNull(),
	slug: text("slug").notNull().unique(),
	orgId: text("org_id")
		.notNull()
		.references(() => org.id, { onDelete: "cascade" }),
	metadata: jsonb("metadata"),
	visibility: callVisibilityEnum("call_visibility")
		.notNull()
		.default("private"),
	createdAt: timestamp("created_at").defaultNow().notNull(),
	updatedAt: timestamp("updated_at").defaultNow().notNull(),
});

export const callToMemberRoleEnum = pgEnum("call_to_member_role", ["admin"]);

export const callToMember = pgTable(
	"call_to_member",
	{
		callId: text("call_id")
			.notNull()
			.references(() => call.id, { onDelete: "cascade" }),
		memberId: text("member_id")
			.notNull()
			.references(() => member.id, { onDelete: "cascade" }),
		role: callToMemberRoleEnum("call_to_member_role")
			.notNull()
			.default("admin"),
	},
	(t) => [primaryKey({ columns: [t.callId, t.memberId] })],
);

export const participant = pgTable(
	"participant",
	{
		id: text("id")
			.$defaultFn(() => nanoid())
			.notNull()
			.primaryKey(),
		userId: text("user_id")
			.notNull()
			.references(() => user.id, { onDelete: "cascade" }),
		callId: text("call_id")
			.notNull()
			.references(() => call.id, { onDelete: "cascade" }),
		createdAt: timestamp("created_at").defaultNow().notNull(),
		updatedAt: timestamp("updated_at").defaultNow().notNull(),
	},
	(t) => [unique().on(t.callId, t.userId)],
);

export const juror = pgTable(
	"juror",
	{
		id: text("id")
			.$defaultFn(() => nanoid())
			.notNull()
			.primaryKey(),
		userId: text("user_id")
			.notNull()
			.references(() => user.id, { onDelete: "cascade" }),
		roundId: text("round_id")
			.notNull()
			.references(() => round.id, { onDelete: "cascade" }),
		createdAt: timestamp("created_at").defaultNow().notNull(),
	},
	(t) => [unique().on(t.userId, t.roundId)],
);

export const submission = pgTable("submission", {
	id: text("id")
		.$defaultFn(() => nanoid())
		.notNull()
		.primaryKey(),
	roundId: text("round_id")
		.notNull()
		.references(() => round.id, { onDelete: "cascade" }),
	participantId: text("participant_id")
		.notNull()
		.references(() => participant.id, { onDelete: "cascade" }),
	data: jsonb("data"),
	createdAt: timestamp("created_at").defaultNow().notNull(),
	updatedAt: timestamp("updated_at").defaultNow().notNull(),
});

export const judgement = pgTable(
	"judgement",
	{
		id: text("id")
			.$defaultFn(() => nanoid())
			.notNull()
			.primaryKey(),
		submissionId: text("submission_id")
			.notNull()
			.references(() => submission.id, { onDelete: "cascade" }),
		jurorId: text("juror_id")
			.notNull()
			.references(() => juror.id, { onDelete: "cascade" }),
		data: jsonb("data"),
		createdAt: timestamp("created_at").defaultNow().notNull(),
	},
	(t) => [unique().on(t.jurorId, t.submissionId)],
);

export const inviteStatusEnum = pgEnum("invite_status", [
	"pending",
	"accepted",
]);

export const memberInvite = pgTable("member_invite", {
	id: text("id")
		.$defaultFn(() => nanoid())
		.notNull()
		.primaryKey(),
	email: text("email").notNull(),
	orgId: text("org_id")
		.notNull()
		.references(() => org.id, { onDelete: "cascade" }),
	role: memberRoleEnum("org_member_role").notNull().default("member"),
	status: inviteStatusEnum("invite_status").notNull().default("pending"),
	invitedBy: text("invited_by")
		.notNull()
		.references(() => member.id),
	createdAt: timestamp("created_at").defaultNow().notNull(),
});

export const jurorInvite = pgTable("juror_invite", {
	id: text("id")
		.$defaultFn(() => nanoid())
		.notNull()
		.primaryKey(),
	email: text("email").notNull(),
	roundId: text("round_id")
		.notNull()
		.references(() => round.id, { onDelete: "cascade" }),
	status: inviteStatusEnum("invite_status").notNull().default("pending"),
	invitedBy: text("invited_by")
		.notNull()
		.references(() => member.id),
	createdAt: timestamp("created_at").defaultNow().notNull(),
});

export const relations = defineRelations(
	{
		user,
		session,
		account,
		verification,
		org,
		member,
		round,
		call,
		callToMember,
		participant,
		juror,
		submission,
		judgement,
		memberInvite,
		jurorInvite,
	},
	(r) => ({
		user: {
			memberProfiles: r.many.member(),
			jurorProfiles: r.many.juror(),
			participantProfiles: r.many.participant(),
		},
		session: {
			user: r.one.user({
				from: r.session.userId,
				to: r.user.id,
				optional: false,
			}),
		},
		account: {
			user: r.one.user({
				from: r.account.userId,
				to: r.user.id,
				optional: false,
			}),
		},
		org: {
			members: r.many.member(),
			calls: r.many.call(),
			invites: r.many.memberInvite(),
		},
		member: {
			org: r.one.org({
				from: r.member.orgId,
				to: r.org.id,
				optional: false,
			}),
			user: r.one.user({
				from: r.member.userId,
				to: r.user.id,
				optional: false,
			}),
		},
		round: {
			call: r.one.call({
				from: r.round.callId,
				to: r.call.id,
				optional: false,
			}),
			jurors: r.many.juror(),
			submissions: r.many.submission(),
		},
		call: {
			org: r.one.org({
				from: r.call.orgId,
				to: r.org.id,
				optional: false,
			}),
			callToMembers: r.many.callToMember(),
			rounds: r.many.round(),
			participants: r.many.participant(),
		},
		callToMember: {
			call: r.one.call({
				from: r.callToMember.callId,
				to: r.call.id,
				optional: false,
			}),
			member: r.one.member({
				from: r.callToMember.memberId,
				to: r.member.id,
				optional: false,
			}),
		},
		participant: {
			call: r.one.call({
				from: r.participant.callId,
				to: r.call.id,
				optional: false,
			}),
			user: r.one.user({
				from: r.participant.userId,
				to: r.user.id,
				optional: false,
			}),
			submissions: r.many.submission(),
		},
		juror: {
			user: r.one.user({
				from: r.juror.userId,
				to: r.user.id,
				optional: false,
			}),
			round: r.one.round({
				from: r.juror.roundId,
				to: r.round.id,
				optional: false,
			}),
			judgements: r.many.judgement(),
		},
		submission: {
			round: r.one.round({
				from: r.submission.roundId,
				to: r.round.id,
				optional: false,
			}),
			participant: r.one.participant({
				from: r.submission.participantId,
				to: r.participant.id,
				optional: false,
			}),
			judgement: r.many.judgement(),
		},
		judgement: {
			submission: r.one.submission({
				from: r.judgement.submissionId,
				to: r.submission.id,
				optional: false,
			}),
			juror: r.one.juror({
				from: r.judgement.jurorId,
				to: r.juror.id,
				optional: false,
			}),
		},
		memberInvite: {
			org: r.one.org({
				from: r.memberInvite.orgId,
				to: r.org.id,
				optional: false,
			}),
			invitee: r.one.member({
				from: r.memberInvite.invitedBy,
				to: r.member.id,
				optional: false,
			}),
		},
		jurorInvite: {
			round: r.one.round({
				from: r.jurorInvite.roundId,
				to: r.round.id,
				optional: false,
			}),
		},
	}),
);
