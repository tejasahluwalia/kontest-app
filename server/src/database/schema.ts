...
export const organizationToHost = pgTable("organization_to_host", {
...
export const organizationToHostRelations = relations(organizationToHost, ({ one }) => ({
...
export const contestToHost = pgTable("contest_to_host", {
...
export const contestToParticipant = pgTable("contest_to_participant", {
    contestId: text('contest_id').notNull().references(() => contest.id, { onDelete: 'cascade' }),
    userId: text('user_id').notNull().references(() => user.id, { onDelete: 'cascade' }),
    createdAt: timestamp('created_at').notNull(),
    updatedAt: timestamp('updated_at').notNull()
...
export const contestToParticipantRelations = relations(contestToParticipant, ({ one }) => ({
    contest: one(contest, {
...
export const jurorToContest = pgTable("juror_to_contest", {
    userId: text('user_id').notNull().references(() => user.id, { onDelete: 'cascade' }),
    contestId: text('contest_id').notNull().references(() => contest.id, { onDelete: 'cascade' }),
    createdAt: timestamp('created_at').notNull(),
    updatedAt: timestamp('updated_at').notNull()
